...
{                             // C_BaseFlex -> C_EconEntity -> C_BaseCombatWeapon -> C_WeaponCSBase -> C_BaseCSGrenade
    ...
    fnInitSendPropOffset(g_iOffset_WeaponWorldModel, "CBaseCombatWeapon", "m_iWorldModelIndex"); /**"m_iWorldModelIndex" not flexible**/
    ...
    //fnInitSendPropOffset(g_iOffset_WeaponReserve1, "CBaseCombatWeapon", "m_iPrimaryAmmoCount");
    //fnInitSendPropOffset(g_iOffset_WeaponReserve2, "CBaseCombatWeapon", "m_iSecondaryAmmoCount");
...
public void WeaponSDKOnFakeSpawnPost(const int referenceIndex) 
{
    ...
    if(weaponIndex != INVALID_ENT_REFERENCE)
    {
        // Validate custom index
        int iD = WeaponsGetCustomID(weaponIndex);
        int ammo_type = GetEntData(weaponIndex, g_iOffset_WeaponAmmoType);
        if(iD != INVALID_ENT_REFERENCE)
        {
            ...
            {/// Set ammo here, because of creating dhook on the next frame after spawn
                SetEntData(weaponIndex, g_iOffset_PlayerAmmo + (ammo_type<<2), iAmmo, _, true); 
...
{ 
    ...
    if(!WeaponsValidateKnife(weaponIndex) && !WeaponsValidateTaser(weaponIndex)) 
    {
        ...
        switch(gClientData[clientIndex][Client_Survivor] ? gCvarList[CVAR_SURVIVOR_INF_AMMUNITION].IntValue : gCvarList[CVAR_HUMAN_INF_AMMUNITION].IntValue)
        {
            case 0 : return;
            // SetEntData(weaponIndex, g_iOffset_PlayerAmmo + (ammo_type<<2), iAmmo, _, true); 
            case 1 : { SetEntData(weaponIndex, (g_iOffset_PlayerAmmo + (ammo_type<<2)), GetEntData(weaponIndex, (g_iOffset_PlayerAmmo + (ammo_type<<2))), _, true); }
...
public Action WeaponSDKOnAmmunition(const int clientIndex, const char[] commandMsg, const int iArguments)
{
    ...
    if(IsPlayerExist(clientIndex))
    {
        ...
        if(IsValidEdict(weaponIndex))
        {
            ...
            int iAmmo = GetEntData(weaponIndex, g_iOffset_PlayerAmmo+(ammo_type<<2));
            int iMaxAmmo = SDKCall(hSDKCallGetReserveAmmoMax, weaponIndex);
            
            // Reset ammomax for standart weapons
            if(!iMaxAmmo) iMaxAmmo = GetEntData(weaponIndex, g_iOffset_PlayerAmmo + (ammo_type<<2)); /// Bug fix for standart weapons
            ...
            if(iAmmo < iMaxAmmo)
            {
                ...
                SetEntData(weaponIndex, g_iOffset_PlayerAmmo+(ammo_type<<2), (iAmmo <= iMaxAmmo) ? iAmmo : iMaxAmmo, _, true);
